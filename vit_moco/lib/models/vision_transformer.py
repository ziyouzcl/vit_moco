"""
@来源: timm==0.9.12 — timm/models/vision_transformer.py（本仓库有修改）
@功能简述: 2D Vision Transformer 主体实现，作为 3D ViT / MoCo 骨干的基础骨架
@主要内容:
    Attention、多种 Transformer Block、VisionTransformer 主类、
    权重初始化、预训练权重转换、模型配置工厂函数

@张量约定（Block / Attention）:
    形状 [B, N, C] — B: batch；N: token 数；C: embed_dim

@本仓库常用（vit_3d_* / MoCo 默认路径）:
    VisionTransformer 主类 + PatchEmbed3D（在 vision_transformer_moco 中注入）
        → 内部堆叠 depth 个 Block（block_fn 默认 Block，未改）
        → 每个 Block 内含 Attention + Mlp
    init_values 默认为 None → Block 内 LayerScale 退化为 nn.Identity()，即通常不用 LayerScale

@模块一览（按使用频率）:
    | 类名                  | 常用? | 说明 |
    |-----------------------|-------|------|
    | Attention             | 是    | 被 Block 调用；Q/K/V 自注意力 |
    | Block                 | 是    | VisionTransformer 默认 block_fn；pre-norm 标准 ViT |
    | Mlp                   | 是    | Block 内 FFN（同文件，未单独列出） |
    | LayerScale            | 可选  | init_values 非 None 时启用；本仓库 3D 工厂未传，一般关闭 |
    | ResPostBlock          | 否    | 需构造时显式 block_fn=ResPostBlock |
    | ParallelScalingBlock  | 否    | 需显式 block_fn；timm 大模型实验用 |
    | ParallelThingsBlock   | 否    | 需显式 block_fn；更实验性 |

@Block（标准，本仓库实际使用）:
    数据流:
        Norm -> Multi-Head Self-Attention -> Residual
             -> Norm -> MLP                  -> Residual
    公式:
        x = x + Attention(LN(x))
        x = x + MLP(LN(x))

@备用 Block（本仓库未用，改模型结构时才考虑）:
    ResPostBlock:          norm 在残差支路后（post-residual）
    ParallelScalingBlock:  Attention 与 MLP 并行后加回残差
    ParallelThingsBlock:   多路 Attention / MLP 并行

@_cfg（文件末尾）:
    timm 模型默认元数据字典工厂；挂到 model.default_cfg，供预训练权重下载、
    推理预处理（input_size / mean / std / crop_pct）及层名映射（first_conv、classifier）使用。
    本仓库 vit_3d_* 在 vision_transformer_moco.py 中写 model.default_cfg = _cfg()，
    多为兼容 timm 习惯；3D CT 实际归一化与尺寸以 Dataset / YAML 为准，勿直接照搬 2D ImageNet 默认值。
"""
...
def _cfg(url: str = '', **kwargs) -> Dict[str, Any]:
    """timm 风格 default_cfg：预训练 URL、输入尺寸、归一化 mean/std、首层/分类头参数名等。"""
    return {
        'url': url,
        'num_classes': 1000,
