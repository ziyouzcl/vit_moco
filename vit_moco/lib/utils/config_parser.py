"""
config_parser — YAML 训练配置解析与实验目录管理

作用
    读取实验 YAML，解析为 EasyDict（支持 args.xxx 点号访问），并自动派生
    checkpoint / 日志路径，可选备份当次配置文件，供 finetuning_main 使用。

主要函数
    | 函数            | 说明                                                       |
    |-----------------|------------------------------------------------------------|
    | get_conf        | 统一入口；conf_parser 取 monai / omega / auto              |
    | get_monai_conf  | MONAI ConfigParser（支持 $import、@ 引用等 Bundle 语法）   |
    | get_omega_conf  | OmegaConf 直读 YAML，结构较简单                            |
    | get_unique_name | 生成带时间戳与主机名的唯一目录名（内部用于 log_dir / 配置备份） |

get_conf 流程
    conf_file
        ↓
    选择解析器（monai / omega / auto）
        ↓
    读取 yaml → 转为 EasyDict
        ↓
    写入 ckpt_dir、log_dir（由 output_root + run_name 派生）
        ↓
    若 save_cfg=True → 备份 YAML 到 output_root/run_name/cfgs/
        ↓
    返回 conf
"""
import argparse
