# -*- coding: utf-8 -*-
"""
@文件名称: vit3d_trainer.py
@功能简述: 3D ViT 有监督训练/微调（分类），封装模型、数据、训练/验证与存盘
@主要用途: 在 MoCo 等预训练权重上做线性探测或端到端微调（CT/MRI 等 3D 体积）
@核心能力:
    1. 从 args.model_obj 挂载 3D ViT，可选加载 pretrain、冻结 backbone
    2. 构建 train / val DataLoader（YAML 中的 train_dataset_obj / val_dataset_obj）
    3. 每 epoch：先 evaluate 再 epoch_train，按 AUC 可选 save_best
    4. AMP 混合精度（GradScaler）
    5. 验证/训练日志：Loss、AUC、AP（torchmetrics），TensorBoard
    6. 断点续训（resume）；定期 checkpoint（含 scaler）

@训练流程:
    初始化 ViT3DTrainer
        → build_model（预训练 / freeze_backbone）
        → build_optimizer（LARS / AdamW / SGD）
        → build_dataloader
        → 每 epoch: evaluate → epoch_train → save_checkpoint

@类方法一览:
    | 方法              | 作用 |
    |-------------------|------|
    | __init__          | super(BaseTrainer)；model_name；GradScaler |
    | build_model       | args.model_obj；load pretrain；可选 freeze；wrap_model |
    | build_optimizer   | get_parameter_groups；lr 按 batch 缩放 |
    | build_dataloader  | train_dataset_obj + val_dataset_obj |
    | run               | 验证 → 训练 → 存盘 |
    | epoch_train       | 单 epoch 训练（model.eval，线性探测协议） |
    | evaluate          | 验证集 Loss / AUC / AP |
    | resume            | 恢复 model + optimizer + start_epoch |

@模块级函数（与 moco3d 同签名，当前 run/epoch_train 未调用，预留）:
    adjust_learning_rate: warmup 后 cosine 衰减

@优化器:
    | 优化器 | 常见用途 |
    |--------|----------|
    | LARS   | 大 batch 自监督 |
    | AdamW  | ViT / Transformer 微调 |
    | SGD    | 传统 CNN 训练 |

@继承基类:
    BaseTrainer — __init__、wrap_model、get_parameter_groups、save_checkpoint

@对外暴露:
    __all__ = ['ViT3DTrainer']

@依赖库:
    torch、timm、torchmetrics、math/os/time
"""
import math
import os
import time
