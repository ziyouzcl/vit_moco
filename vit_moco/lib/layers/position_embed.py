"""
position_embed — ViT / 3D ViT 位置编码构建

作用
    为 patch token 序列提供空间位置信息，使 Transformer 能区分不同 patch 的空间关系。

主要函数
    | 函数                               | 维度 | 是否可训练 | 说明                                      |
    |------------------------------------|------|------------|-------------------------------------------|
    | build_2d_sincos_position_embedding | 2D   | 否         | 正弦/余弦固定位置编码，grid 默认各向同性     |
    | build_3d_sincos_position_embedding | 3D   | 否         | 3D sin-cos，embed_dim 需被 6 整除           |
    | build_perceptron_position_embedding| 2D/3D| 是         | 随机初始化，可学习的位置编码                 |

使用建议
    3D CT/MRI 的 MoCo / ViT-MoCo（如 VisionTransformerMoCo3D）通常采用
    build_3d_sincos_position_embedding：固定、稳定，利于自监督预训练且不易过拟合。
"""
